...
def chrome_goodreads_page(title, author)
  @goodreads_driver ||= init_chromium_driver # set up chrome instance if it's not open
  request_url = goodreads_url(title, author)
  @goodreads_driver.navigate.to request_url
end

def goodreads_not_found(parsed_page)
  (parsed_page.xpath("//h3[contains(., 'No results')]").to_a != []) ||
...
def get_goodreads_info # Manually (from operator) Get the title, author, and rating actually displayed on page
  ...
  { goodreads_title:, goodreads_author:, stars: stars_string.to_f,
    count: ratings_string.to_i, match: true }
end
...
def author_strings(author)
  ...
  search_strings = []
  search_strings << "#{first} #{middle} #{last}" if middle > ''
...
def punct_strip(s)
  s = Regexp.last_match(1) if s =~ /(.*?)([ .,;:\-?$!]*$)/ # Ending punctuation
  s
end

def title_strings(title)
  title = Regexp.last_match(1) if title =~ /(.*): a novel/i
  title = Regexp.last_match(5) if title =~ /^((The )|(A )|(An ))(.*)/ # Strip initial The, A, and An
...
def original_or_goodreads_title(book)
  ...
  t = g if g && (g > '') && (g != 'ignore') && (g != 'no match xxx')
  t
end
...
def goodreadsRating(book)
  ...
  if gr_auth > ''
    ...
    author = book[:author] || ''
    author = Regexp.last_match(1) if author =~ /(.*) \(.*\)/ # Get rid of parenthesized stuff in author name
    author_orig = author.clone
    author = Regexp.last_match(1) if author =~ /(.* \w{2,})\./ # Get rid of trailing period that's not for an initial
  end
  ...
  if not_found
    ...
    return { goodreads_title: 'no match xxx', match: false } unless $manual_update
  ...
  if ratings
    ...
    rating_info = { stars:, count:, goodreads_title: title, goodreads_author: author_try,
                    match: true }
  else
...
def get_stars(ratings) # Find highest rated entry among the array of matching minirating spans
  ...
  { stars: max_stars, count: max_count }
end
